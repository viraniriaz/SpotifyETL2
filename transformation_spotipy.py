...
job = Job(glueContext)
s3_path = "s3://spotify-etl-project-riaz/raw_data/to_processed/"
...
def write_to_s3(df, path_suffix, format_type="csv"):
    ...
    glueContext.write_dynamic_frame.from_options(
        frame = dynamic_frame,
        connection_type = "s3",
        connection_options = {"path": f"s3://spotify-etl-project-riaz/transformed_data/{path_suffix}/"},
        format = format_type
    )
...
song_df.show()
write_to_s3(song_df, "song_data/songs_transformed_{}".format(datetime.now().strftime("%Y-%m-%d")), "csv")
job.commit()